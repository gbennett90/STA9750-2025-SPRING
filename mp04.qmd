...
# Calculate total vote increase
biggest_increase <- joined_data %>%
  filter(!is.na(total_votes_2020), !is.na(total_votes_2024)) %>%
  mutate(vote_change = total_votes_2024 - total_votes_2020) %>%
  arrange(desc(vote_change)) %>%
  slice(1:10) %>%
  select(state, county, total_votes_2020, total_votes_2024, vote_change)

kable(biggest_increase, caption = "Top 10 Counties with the Largest Increase in Total Votes (2020 to 2024)")
```
