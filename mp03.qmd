...
if (nrow(zero_appearance_tracks) == 0) {
  kable(
    tibble(Message = "✅ No tracks with 0 appearances."),
...
These anchor tracks were chosen for their distinct yet complementary energy profiles. 

**"Blinding Lights"** offers a pulsating synth-driven groove, while **"ROCKSTAR"** brings a gritty, rhythmic edge, together setting the stage for diverse playlist exploration.
...
**Playlist Co-occurrence:** Identified songs most frequently paired with each anchor across user-created playlists, revealing natural pairings.
...
**Non-Popular:** Ensured at least 3 songs with popularity scores below 50 to diversify beyond mainstream hits.
...
# Ensure all 12 songs are represented (fill with NA if missing)
...
***
### 🏁 Conclusion
**"The Ultimate Workout Journey"** playlist blends data an
